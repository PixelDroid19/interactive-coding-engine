...
## Task 5k: Delegated catalog routes (76, in progress)
...
- [ ] Connect the module-owned route resolution to an actual shell navigation and document the boundary with the public router.
- [ ] Validate catalog home/detail, rejected URLs and parameter propagation in the playground and exported application.

Resolver evidence: two execution tests failed on the original artifact (prefix collision and undecoded identifier). The generated function now rejects ambiguous paths and returns a typed home/detail descriptor. This is an application-owned helper, not a new Open Cells API, and it is not yet connected to navigation. Narration and subtitle sources are unchanged.
