...
## Task 5j: Pending-change navigation decisions (75, implemented)
...
- [x] Connect the policy to the actual application/router boundary, with one effective transition and stale-confirmation protection.
- [x] Exercise the confirmation UI, cancellation, accepted navigation and repeated attempts in the playground and the exported application.
...
Application integration: lesson 75 now adds a translated session-note editor and native modal to the application shell. The synchronous interceptor delegates the deferred answer to the tested coordinator via the actual `{ value: navigation }` channel. Cancellation and Escape retain the draft; acceptance restores the saved session value and resumes the requested route; newer intents replace older pending dialogs. Locale catalogs and SCSS/runtime styles travel in the ZIP. Audio, subtitles and other lesson workspaces are unchanged.

Acceptance evidence: `scripts/verify-cells-navigation-browser.ts` first failed on the missing note editor, then passed cancellation, acceptance, saved-note navigation, Spanish/English switching, Escape, mobile rendering and a newer search intent superseding Favorites. The same script passes against both the playground and the production export using Open Cells 1.2.1 (`CELLS_NAVIGATION_APP_URL`). Actual export `cells app:test` and `cells app:build -c prod.js` pass. That gate exposed an unrelated inherited test expecting the store id `tea`; the shared test now selects an explicit fixture product and proves its id reaches navigation for every project. Host checks: 36 targeted tests, TypeScript and production build pass; the build still reports existing Pyodide externalization and large-chunk warnings. Saved notes last for this app session only; this example guards in-app routes, not browser-tab closure. Next: implementations 76–84.

## Task 5k: Delegated catalog routes (76, completed)

- [x] Define the internal URL resolver boundary: exact catalog prefix, one decoded identifier, no foreign URL, extra segment or malformed encoding.
- [x] Connect the module-owned route resolution to an actual shell navigation and document the boundary with the public router.
- [x] Validate catalog home/detail, rejected URLs and parameter propagation in the playground and exported application.

Resolver evidence: two execution tests failed on the original artifact (prefix collision and undecoded identifier). The generated function now rejects ambiguous paths and returns a typed home/detail descriptor. This is an application-owned helper, not a new Open Cells API. Narration and subtitle sources are unchanged.

The shell now calls the catalog-owned navigation adapter, which translates that descriptor into a public named route. The catalog owns its route definitions in `app/modules/catalogo/routes.js`; the shell composes them with its other routes. The playground demonstrates navigation inside its isolated frame, not address-bar deep linking. The exported Open Cells application additionally proves shareable hash URLs, reload, fresh browser context, Back and Forward. Both runtimes pass decoded identifiers, literal percent encoding, invalid links preserving the current page, Spanish/English switching and mobile checks through `scripts/verify-cells-delegated-routes-browser.ts`.

Native verification exposed shared recipe corrections: each page supplies its private channel, inactive/cached pages are hidden, and route parameters arrive through `this.params` before argument-free `onPageEnter()`. The preview now follows that parameter contract. Exported CLI tests/build and lesson 75 navigation regression passed; the final focused rerun has 22 passing tests. Root TypeScript and production build passed (existing Pyodide/chunk warnings remain). Manual production-player verification covered user-gesture start, pause, completed tape writes, catalog deep link, editor fork, preview rebuild and return to roadmap. Next: implementations 77–84; this does not complete the overall course migration.

## Task 5l: Bounded page retention (77, in progress)

Follow-up regression correction for 76: the full generated-tape audit rejected the catalog module because its static preflight searched only the shell and hardcoded `/product/:id`. It now follows locally imported spread tables and recognizes the named parameterized detail route without requiring that URL spelling. A regression proves a disconnected catalog module still fails. The combined audit, artifact and course/guided suites pass 40 tests. The preliminary source audit remains separate from actual browser behavior.

- [x] Align eviction with the lesson's `cleanup` contract and reject non-positive, fractional and non-finite limits.
- [ ] Connect retention to an observable page lifecycle without confusing the application-owned helper with a public Open Cells API.
...
## Main-player visibility correction

The real player exposed a delivery gap that workspace tests could not detect: `teachingFilePaths` hid supplemental documentation and the application-owned modules. The player now offers **Todo el proyecto**, preserving focused lesson files as the default. The explorer exposes the complete workspace; tabs remain limited to lesson files plus the selected supplemental file. Returning to **Solo la clase** selects a lesson file without editing or deleting content. Changing lesson resets the view.

The regression first failed because the action was missing. It now verifies opening a supplemental document, returning to the focused file, absence of a learner branch from browsing alone, and reset on lesson change. All 28 player tests pass; TypeScript/build pass. The full suite passes 1,379 tests with 2 skipped.

Browser evidence: on the real player, lesson 80 started by gesture, advanced, paused, opened its guide, reconstructed the final tape via the progress control, ran the app, navigated to Museo and back, forked on a comment edit, reran, preserved the edit when cancelling the return dialog, saved on exit, and opened its separate reasoning surface. At 390px, lesson 81 started/paused and opened its guide with the preview docked; the floating preview can cover the editor at that width, so that mode remains a separate layout limitation. A fresh production build on local port 4199 passed lesson 84 start/pause, final-tape reconstruction, both migration consumers, retirement, detail/back and entry into reasoning, with no browser console errors. These are interaction checks, not a complete listen-through of every recording.

Browser routing: the in-app browser bootstrap failed with `privileged native pipe bridge is not available; browser-client is not trusted`; the checks used isolated agent-browser Chromium sessions. Existing development servers were preserved. The initial port-3000 server showed an invalid React hook error; checks continued on the working port-3001 server and the fresh production build, without claiming port 3000 was repaired.

## Latest acceptance evidence (2026-09-08, implementation `3348de5`)

- Main production player, lesson 81: final-tape application selection reaches detail and returns; the collector records the minimal versioned event and rejects the invalid event without appending it. The separate reasoning surface opens.
- Lesson 82: selection/detail/return records an observed 19.90 ms against the 250 ms budget. Mounted pages report one against two. JavaScript and separate CSS remain explicitly unmeasured in the in-memory playground; this does not establish exported resource sizes.
- Lesson 83: imported actual successful and failed `release:check` report files through the browser's native file input. The successful report displays five approved gates and matching execution/source/artifact identities. The failed report displays failed tests, no artifact identity and three unexecuted downstream gates. The local-report authenticity warning remains visible. No browser errors were recorded.
- Mobile floating preview: the regression failed at the old 96 px top position before correction. At 390 × 844, the rebuilt player leaves its wrapped header and project controls above the preview and its playback controls below. Opening the full project succeeds. Desktop placement remains 52 px from the top at 1280 × 800. This corrects initial placement, not every possible user-dragged overlap.
- Fresh full suite: 1,379 passed and two skipped with `--maxWorkers=2 --reporter=dot`. The unconstrained run timed out in the existing five-second tape audit; no timeout was increased. The two structural CLI parity tests also pass when run separately with the installed local CLI root. These compare recipe structure and versions, not all exported runtime behavior.
