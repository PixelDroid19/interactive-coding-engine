...
export function auditCellsComponent(workspace: WorkspaceSnapshot): { results: CellsTestResult[]; coverage: CellsCoverageResult } {
  ...
  const consumesGeneratedStyles = Boolean(
    ...
    && new RegExp(`import\\s+styles\\s+from\\s+['"]\\./${tagName}\\.css\\.js['"]`).test(source)
    && /static\s+styles\s*=\s*styles\s*;/.test(source)
  ...
  const results = [
    ...
    test('public-property', 'Declara la entrada pública learnerName', /learnerName\s*:\s*\{\s*type:\s*String\s*,\s*attribute:\s*['"]learner-name['"]/.test(source), 'learnerName debe ser una propiedad String configurable también mediante learner-name.', sourcePath),
    test('scoped-components', 'Registra dependencias scoped', /['"]bbva-type-text['"]\s*:\s*BbvaTypeText/.test(scopedRegistry) && /['"]bbva-button-default['"]\s*:\s*BbvaButtonDefault/.test(scopedRegistry), 'Registra los componentes elegidos del catálogo dentro de scopedElements.', sourcePath),
    ...
    test('demo-controls-property', 'La demo configura la propiedad pública', /card\.learnerName\s*=\s*event\.target\.value/.test(demoController), 'El control de nombre debe modificar learnerName en la instancia, no una copia interna.', 'demo/demo.js'),
    ...
    test('test-public-event', 'Prueba el evento desde la API pública', /expect\(event\.detail\)\.toEqual\(/.test(componentTest) && /expect\(event\.bubbles\)\.toBe\(true\)/.test(componentTest) && /expect\(event\.composed\)\.toBe\(true\)/.test(componentTest), 'La suite debe comprobar detail, bubbles y composed a partir del evento recibido por un consumidor.', tagName ? `test/unit/${tagName}.test.js` : undefined),
...
export function auditCellsApplication(workspace: WorkspaceSnapshot): { results: CellsTestResult[]; coverage: CellsCoverageResult } {
  ...
  const results = [
    ...
    test('native-boundary', 'Traduce mensajes externos en una frontera', /typeof\s+message\.type\s*!==\s*['"]string['"]/.test(nativeAdapter) && /publish\(APP_LIFECYCLE_CHANNEL\s*,\s*\{\s*state:\s*message\.state\s*\}\)/.test(nativeAdapter) && /navigate\(message\.route\s*,\s*message\.params\s*\?\?\s*\{\}\)/.test(nativeAdapter), 'El adaptador debe validar el mensaje y traducir ciclo o deep link a contratos internos.', nativeAdapterPath),
