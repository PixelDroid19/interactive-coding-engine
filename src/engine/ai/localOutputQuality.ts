...
export function validateSpanishGeneration(text: string): string | null {
  ...
  const wrongLanguageForSpanishLab = normalized.length > 40 && spanishSignals < 2;

  if (hasReplacementCharacter || hasPunctuationRun || hasRepeatedChunk || hasUnexpectedScripts || wrongLanguageForSpanishLab) {
