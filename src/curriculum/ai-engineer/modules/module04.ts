...
print(elegir_despliegue(True, False))`, starter: `def elegir_despliegue(sin_red, datos_sensibles):
    ...
    reading: { core: 'Inferencia local ocurre en navegador, móvil, escritorio o infraestructura propia. Inferencia alojada usa una API. También existen despliegues híbridos que enrutan según tarea y riesgo.', mechanics: 'Local descarga artefactos y usa CPU, GPU o aceleradores. Cuantización reduce memoria a cambio de posibles cambios de calidad. Alojar delega escalado, actualizaciones y parte de la observabilidad.', decisions: 'Compara tiempo de primera carga, latencia sostenida, privacidad, coste por uso, disponibilidad sin red y capacidad del equipo. Prueba en dispositivos reales.', errors: 'Local no significa automáticamente privado si la aplicación envía telemetría o descarga datos externos. Alojar no elimina responsabilidades de consentimiento y retención.', keyPoints: ['Mide en el dispositivo objetivo.', 'Local y alojado pueden coexistir.', 'La privacidad depende del flujo completo.'], question: '¿Un modelo en el navegador funciona sin internet?', answer: 'Después de descargar y cachear artefactos puede hacerlo, según implementación. La primera carga y modelos no cacheados sí necesitan red.', transfer: 'Diseña un enrutador para una app móvil: qué tareas harías localmente y cuáles enviarías a una API.', sources: ['transformers-js', 'hf-model-hub', 'pyodide-worker'] },
...
print(tarea_pipeline("embeddings"))`, starter: `def tarea_pipeline(necesidad):
    ...
    reading: { core: 'El Hub alberga pesos, configuración, tokenizador, código, datasets y model cards. Un repositorio puede tener varias revisiones y formatos. La model card describe uso previsto y límites, pero debes comprobarla.', mechanics: 'Transformers.js resuelve artefactos, los cachea y ejecuta una pipeline con ONNX Runtime Web. El modelo se carga bajo demanda en un Worker para no bloquear la interfaz.', decisions: 'Revisa licencia, tarea, idiomas, tamaño, cuantización y compatibilidad web. Fija una revisión cuando necesites reproducibilidad. Mide primera carga y uso posterior.', errors: 'Copiar un id sin leer la card puede cargar una tarea incorrecta o un archivo enorme. Confiar en latest sin control cambia comportamiento. Un modelo compatible con Python no siempre tiene artefactos web.', keyPoints: ['La model card forma parte del contrato.', 'La tarea determina pre y posprocesamiento.', 'La carga local necesita progreso, caché y fallback.'], question: '¿El modelo se descarga en cada visita?', answer: 'El navegador puede cachearlo, pero depende de almacenamiento, origen y políticas. La interfaz debe funcionar también durante descarga o si la caché se borra.', transfer: 'Elige una model card y anota tarea, idioma, licencia, tamaño, métrica y una limitación.', sources: ['hf-model-hub', 'transformers-js'] },
...
print(solicitud_modelo("local", "hola"))`, starter: `def solicitud_modelo(modelo, mensaje):
    ...
    reading: { core: 'Un SDK ofrece tipos, reintentos y helpers, pero acopla al contrato del proveedor. Un adaptador interno define mensajes, resultados y errores que necesita tu producto y traduce en el borde.', mechanics: 'El adaptador construye endpoint, headers y body, envía con timeout y señal de cancelación, valida estado, parsea cuerpo y normaliza texto, uso y errores.', decisions: 'Expón solo capacidades que realmente soportas. Una “compatibilidad OpenAI” puede cubrir chat pero no tools o streaming. Prueba cada capacidad y conserva flags explícitos.', errors: 'Una abstracción demasiado grande oculta diferencias importantes. Registrar solicitudes completas puede filtrar claves o datos. Reintentar acciones no idempotentes duplica efectos.', keyPoints: ['Tu producto depende de un contrato interno pequeño.', 'Las capacidades se detectan y prueban.', 'En producción las claves viven en backend.'], question: '¿Por qué permitir APIs en navegador durante el curso?', answer: 'Para experimentar con una clave limitada y temporal. La interfaz advierte que es visible. El despliegue real usa un backend con autenticación, cuotas y secretos del servidor.', transfer: 'Define tres métodos de un proveedor interno y un error normalizado que tu interfaz pueda mostrar.', sources: ['openai-function-calling', 'google-function-calling', 'anthropic-tool-use'] },
