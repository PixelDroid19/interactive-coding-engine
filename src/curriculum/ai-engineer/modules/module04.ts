...
print(tarea_pipeline("embeddings"))`, starter: `def tarea_pipeline(necesidad):
    ...
    reading: { core: 'El Hub alberga pesos, configuración, tokenizador, código, datasets y model cards. Transformers.js es útil para muchas tareas en navegador; para este laboratorio generativo usamos WebLLM y el artefacto MLC de Qwen2.5-0.5B-Instruct. Así el texto que aparece proviene del modelo ejecutado realmente en la GPU, no de una simulación.', mechanics: 'WebLLM descarga el modelo cuantizado bajo demanda, conserva sus archivos en la caché del navegador y ejecuta la inferencia WebGPU dentro de un Worker. La interfaz recibe progreso de carga y fragmentos de texto en streaming. Cuando la práctica exige un objeto, activa el modo JSON nativo mediante response_format y un esquema de claves; después vuelve a comprobar el resultado con JSON.parse. El modo estructurado guía la generación, pero la aplicación sigue siendo responsable de validar el contrato.', decisions: 'Revisa licencia, tarea, idiomas, memoria GPU estimada, cuantización y compatibilidad web. Fija una revisión cuando necesites reproducibilidad. Mide por separado primera carga y generación posterior. Una model card no sustituye la prueba en el dispositivo objetivo.', errors: 'Copiar un id sin leer la card puede cargar una tarea incorrecta o un archivo enorme. Confiar en latest sin control cambia comportamiento. Si WebGPU no está disponible o falla, el laboratorio debe informar el error real; no debe inventar una respuesta ni ejecutar silenciosamente otra ruta. Si la salida incumple formato o idioma, muéstrala con una advertencia para poder analizar el fallo.', keyPoints: ['La model card forma parte del contrato.', 'WebLLM ejecuta el modelo generativo mediante WebGPU.', 'La carga local necesita consentimiento, progreso y caché visibles.'], question: '¿El modelo se descarga en cada visita?', answer: 'El navegador puede conservarlo en caché, pero depende del almacenamiento, el origen y sus políticas. La interfaz muestra ese estado antes de ejecutar.', transfer: 'Elige una model card y anota tarea, idioma, licencia, memoria estimada, métrica y una limitación.', sources: ['hf-model-hub', 'transformers-js', 'transformers-js-v4', 'webllm', 'qwen25-webllm', 'qwen25-05b', 'lfm25-350m', 'codepen-transformers-js'] },
