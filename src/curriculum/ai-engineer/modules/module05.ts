...
print(misma_dimension([0.1, 0.2], [0.4, 0.5]))`, starter: `def misma_dimension(a, b):
    ...
    reasoning: { activity: decisionActivity('Decide qué comparaciones tienen contrato.', [['mismo', 'Mismo modelo, revisión y dimensión', ['válida', 'inválida'], 'válida'], ['distinto', 'Modelos distintos con 384 dimensiones', ['válida', 'inválida'], 'inválida'], ['vacio', 'Dos listas vacías', ['válida', 'inválida'], 'inválida']]), explanation: 'Compartir dimensión no basta. La representación debe provenir del mismo contrato de modelo y preprocesamiento.', hints: ['Revisa el origen y el tamaño.', 'Vacío no representa contenido.'] },
  ...
  authoredLesson({
    ...
    summary: 'Carga un modelo multilingüe bajo demanda, informa progreso y calcula embeddings reales mediante WebGPU.',
    concepts: [['Feature extraction', 'Pipeline que produce características o embeddings.'], ['Inferencia local', 'Cálculo ejecutado en el navegador sin enviar el texto a una API.'], ['WebGPU', 'Interfaz que permite ejecutar el modelo con la GPU del dispositivo.']],
...
def normalizar_vector(vector):
    ...
    reading: { core: 'Transformers.js puede ejecutar una pipeline de feature extraction en el navegador. El Worker evita bloquear React. El modelo se carga solo cuando la persona inicia la práctica.', mechanics: 'El servicio envía un lote de textos, recibe progreso de archivos, ejecuta mean pooling y normalización con WebGPU, y devuelve vectores en el mismo orden. Una señal de cancelación descarta la petición.', decisions: 'Elige un modelo pequeño y multilingüe para el laboratorio. Muestra id, dispositivo y estado. Si WebGPU falla, informa el error y conserva el último resultado válido; no construyas vectores falsos.', errors: 'Un vector hash no es un embedding semántico. Presentarlo como búsqueda semántica engaña. Cargar el modelo al iniciar toda la aplicación penaliza a estudiantes que aún no lo necesitan.', keyPoints: ['El modelo se carga bajo demanda en un Worker.', 'El orden del lote se conserva.', 'El ranking usa únicamente embeddings WebGPU reales.'], question: '¿El texto sale del navegador?', answer: 'No se envía a una API de modelo. La descarga inicial de los artefactos sí requiere red; después la caché puede permitir reutilizarlos.', transfer: 'Diseña estados para WebGPU ausente, descarga, inferencia y resultado. Escribe qué puede afirmar cada uno.', sources: ['transformers-js', 'transformers-js-v4', 'hf-model-hub'] },
