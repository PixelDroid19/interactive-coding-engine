...
describe('curso Open Cells', () => {
  ...
  it('entrega 68 unidades progresivas y cinco laboratorios verticales reales', () => {
    ...
    expect(items.filter((item) => item.type === 'reading' && item.handsOnLab)).toHaveLength(5);
    ...
    expect(allItems.filter((item) => item.type === 'debugging')).toHaveLength(68);
  });
  ...
  it('incluye una primera clase guiada que prepara el laboratorio real', () => {
    ...
    expect(items.slice(lessonIndex + 1).some((item) => item.type === 'debugging' && item.relatedLessonId === lesson.id)).toBe(true);
  });
});
