...
export const COMPONENT_SPECS_33_TO_40 = [
  lesson({
    ...
    example: `import{LitElement,html}from'lit';import{Directive,directive}from'lit/directive.js';class AriaCurrentDirective extends Directive{update(part,[active]){part.element.toggleAttribute('aria-current',active);return active?'Paso actual':'Paso';}render(active){return active?'Paso actual':'Paso';}}const ariaCurrent=directive(AriaCurrentDirective);class StepNav extends LitElement{render(){return html\`<button>\${ariaCurrent(true)}</button>\`;}}customElements.define('step-nav',StepNav);`,
    starter: `import{LitElement,html}from'lit';import{Directive,directive}from'lit/directive.js';
class InvalidFieldDirective extends Directive{
  update(part,[invalid,message]){/* verifica una ChildPart, actualiza aria-invalid del input anterior y devuelve el mensaje */}
  render(invalid,message){/* devuelve mensaje o cadena vacía */}
}
const invalidField=directive(InvalidFieldDirective);
class ValidationForm extends LitElement{static properties={email:{state:true}};constructor(){super();this.email='';}render(){const invalid=this.email.length>0&&!this.email.includes('@');return html\`<label>Correo <input .value=\${this.email} @input=\${event=>this.email=event.target.value}></label><p role="alert">\${invalidField(invalid,'Escribe un correo válido')}</p>\`;}}
customElements.define('validation-form',ValidationForm);`,
    ...
    debug: { title: 'La directiva conserva el primer valor para siempre', expected: 'live-label responde a cada cambio de label.', observed: 'render ignora actualizaciones después de la primera.',
      starter: `import{LitElement,html}from'lit';import{Directive,directive}from'lit/directive.js';class StickyLabel extends Directive{render(value){if(this.saved===undefined)this.saved=value;return this.saved;}}const stickyLabel=directive(StickyLabel);class LiveLabel extends LitElement{static properties={label:{type:String}};constructor(){super();this.label='Uno';}render(){return html\`<p>\${stickyLabel(this.label)}</p>\`;}}customElements.define('live-label',LiveLabel);`,
  ...
  lesson({
    ...
    example: `import{LitElement,html}from'lit';class RevealCard extends LitElement{async reveal(){this.open=true;await this.updateComplete;const card=this.shadowRoot.querySelector('article');if(!matchMedia('(prefers-reduced-motion: reduce)').matches)card.animate([{opacity:0},{opacity:1}],{duration:180});}render(){return html\`<button @click=\${()=>this.reveal()}>Mostrar</button>\${this.open?html\`<article>Contenido</article>\`:null}\`;}}customElements.define('reveal-card',RevealCard);`,
    starter: `import{LitElement,html}from'lit';class NoticeStack extends LitElement{static properties={notices:{state:true}};constructor(){super();this.notices=[];}
  async add(message){/* agrega con id, espera el render y anima solo si la preferencia lo permite */}
  remove(id){this.notices=this.notices.filter(notice=>notice.id!==id);}
  render(){return html\`<button @click=\${()=>this.add('Guardado')}>Avisar</button><section aria-live="polite">\${this.notices.map(n=>html\`<article data-id=\${n.id}>\${n.message}<button @click=\${()=>this.remove(n.id)}>Cerrar</button></article>\`)}</section>\`;}}
customElements.define('notice-stack',NoticeStack);`,
    ...
    debug: { title: 'La tarjeta intenta animarse antes de existir', expected: 'detail-panel espera updateComplete antes de animate.', observed: 'querySelector devuelve null al abrir.',
      starter: `import{LitElement,html}from'lit';class DetailPanel extends LitElement{static properties={open:{state:true}};constructor(){super();this.open=false;}show(){this.open=true;this.shadowRoot.querySelector('article').animate([{opacity:0},{opacity:1}],150);}render(){return this.open?html\`<article>Detalle</article>\`:html\`<button @click=\${()=>this.show()}>Abrir</button>\`;}}customElements.define('detail-panel',DetailPanel);`,
  ...
  lesson({
    ...
    example: `import{LitElement,html}from'lit';class Store{listeners=new Set();subscribe(fn){this.listeners.add(fn);return()=>this.listeners.delete(fn);}emit(value){for(const fn of this.listeners)fn(value);}}const store=new Store();class StoreView extends LitElement{connectedCallback(){super.connectedCallback();this.stop=store.subscribe(value=>{this.value=value;this.requestUpdate();});}disconnectedCallback(){this.stop?.();super.disconnectedCallback();}render(){return html\`<p>\${this.value??'Sin datos'}</p>\`;}}customElements.define('store-view',StoreView);`,
    starter: `import{LitElement,html}from'lit';class MetricSource{constructor(){this.listeners=new Set();}subscribe(listener){/* registra y devuelve unsubscribe */}publish(metric){/* notifica una instantánea a cada listener */}}
const metrics=new MetricSource();class MetricsBoard extends LitElement{static properties={latest:{state:true}};constructor(){super();this.latest=null;}
  connectedCallback(){super.connectedCallback();/* suscribe y guarda cleanup */}
  disconnectedCallback(){/* limpia y llama super */}
...
customElements.define('metrics-board',MetricsBoard);`,
    ...
    debug: { title: 'Cada conexión duplica las notificaciones', expected: 'feed-view conserva una sola suscripción y la cancela.', observed: 'connectedCallback añade listeners que nunca salen.',
      starter: `import{LitElement,html}from'lit';const feed=new EventTarget();class FeedView extends LitElement{connectedCallback(){super.connectedCallback();feed.addEventListener('message',event=>{this.message=event.detail;this.requestUpdate();});}render(){return html\`<p>\${this.message??'Vacío'}</p>\`;}}customElements.define('feed-view',FeedView);`,
  ...
  lesson({
    ...
    example: `import{LitElement,html}from'lit';class WeatherAdapter{constructor(client){this.client=client;}async current(city){const raw=await this.client.fetchCity(city);return{temperature:Number(raw.temp_c),label:raw.condition};}}class WeatherCard extends LitElement{constructor(){super();this.service=new WeatherAdapter({fetchCity:async()=>({temp_c:'20',condition:'Claro'})});}async load(){this.weather=await this.service.current('Bogotá');this.requestUpdate();}render(){return html\`<button @click=\${()=>this.load()}>Cargar</button><p>\${this.weather?.temperature??'--'}</p>\`;}}customElements.define('weather-card',WeatherCard);`,
    starter: `import{LitElement,html}from'lit';class PaymentAdapter{constructor(provider){this.provider=provider;}async charge(amount){/* valida, llama provider.pay y normaliza a {success,message,id} */}}
const demoProvider={pay:async cents=>({ok:true,transaction:'tx-1',note:\`\${cents} centavos\`})};
class PaymentPanel extends LitElement{static properties={result:{state:true},pending:{state:true}};constructor(){super();this.service=new PaymentAdapter(demoProvider);this.result=null;this.pending=false;}async submit(amount){/* estados y llamada al contrato propio */}render(){return html\`<button ?disabled=\${this.pending} @click=\${()=>this.submit(1250)}>Pagar</button><p role="status">\${this.pending?'Procesando':this.result?.message??'Sin pago'}</p>\`;}}
customElements.define('payment-panel',PaymentPanel);`,
    ...
    debug: { title: 'El componente depende de los campos crudos del proveedor', expected: 'shipping-card llama a un adapter que devuelve cost.', observed: 'render conoce total_cents y divide manualmente.',
      starter: `import{LitElement,html}from'lit';const provider={quote:async()=>({total_cents:950,currency_code:'USD'})};class ShippingCard extends LitElement{async load(){this.raw=await provider.quote();this.requestUpdate();}render(){return html\`<button @click=\${()=>this.load()}>Cotizar</button><p>\${this.raw?this.raw.total_cents/100:'--'}</p>\`;}}customElements.define('shipping-card',ShippingCard);`,
  ...
  lesson({
    ...
    example: `import{LitElement,html}from'lit';class BookService{constructor(client){this.client=client;}async search(term){const rows=await this.client(term);return rows.map(row=>({id:String(row.key),title:row.name||'Sin título'}));}}class BookShelf extends LitElement{constructor(){super();this.service=new BookService(async()=>[{key:1,name:'Algoritmos'}]);this.books=[];}async load(){this.books=await this.service.search('web');this.requestUpdate();}render(){return html\`<button @click=\${()=>this.load()}>Buscar</button>\${this.books.map(b=>html\`<p>\${b.title}</p>\`)}\`;}}customElements.define('book-shelf',BookShelf);`,
    starter: `import{LitElement,html}from'lit';import{Task}from'@lit/task';class MuseumService{constructor(client){this.client=client;}async search(query,{signal}={}){/* consulta client y devuelve {id,title,artist,year,imageUrl} normalizados */}}
...
class MuseumRoom extends LitElement{static properties={query:{state:true},favorites:{state:true}};constructor(){super();this.query='jardín';this.favorites=[];this.service=new MuseumService(fixtureClient);this.searchTask=new Task(this,{task:([query],options)=>this.service.search(query,options),args:()=>[this.query]});}
 toggleFavorite(id){/* actualiza ids sin mutar */}
 render(){/* formulario de búsqueda + pending/empty/error/ready; tarjetas semánticas y favoritos */}}
customElements.define('museum-room',MuseumRoom);`,
    ...
    debug: { title: 'La sala rompe cuando una obra no tiene imagen', expected: 'art-card conserva título y muestra un fallback.', observed: 'Accede a image.url sin comprobar image.',
      starter: `import{LitElement,html}from'lit';class ArtCard extends LitElement{static properties={art:{attribute:false}};render(){return html\`<img src=\${this.art.image.url} alt=\${this.art.title}><h2>\${this.art.title}</h2>\`;}}customElements.define('art-card',ArtCard);`,
  ...
  lesson({
    ...
    example: `import{LitElement,html}from'lit';class PriceBoard extends LitElement{constructor(){super();this.results=[];}async load(){const settled=await Promise.allSettled([Promise.resolve({id:'a',value:10}),Promise.reject(new Error('Sin red'))]);this.results=settled.map((result,index)=>result.status==='fulfilled'?{...result.value,status:'ready'}:{id:String(index),status:'error'});this.requestUpdate();}render(){return html\`<button @click=\${()=>this.load()}>Cargar</button>\${this.results.map(r=>html\`<p>\${r.status}</p>\`)}\`;}}customElements.define('price-board',PriceBoard);`,
    starter: `import{LitElement,html}from'lit';class WeatherService{constructor(client){this.client=client;}async current(city,unit){/* normaliza a {id,city,temperature,unit,condition} */}}
const fixture=async city=>{if(city==='Error')throw new Error('No disponible');return{name:city,tempC:20,condition:'Claro'};};
class WeatherDashboard extends LitElement{static properties={cities:{state:true},results:{state:true},unit:{state:true}};constructor(){super();this.cities=['Bogotá','Error','Lima'];this.results=[];this.unit='C';this.service=new WeatherService(fixture);}async refresh(){/* allSettled y una fila ready/error por ciudad */}render(){/* control de unidad, actualizar y lista que no borra éxitos */}}
customElements.define('weather-dashboard',WeatherDashboard);`,
    ...
    debug: { title: 'Una ciudad fallida vacía todo el tablero', expected: 'city-board conserva resultados individuales.', observed: 'Promise.all rechaza el grupo completo.',
      starter: `import{LitElement,html}from'lit';class CityBoard extends LitElement{constructor(){super();this.loaders=[()=>Promise.resolve('Bogotá'),()=>Promise.reject(new Error('fallo'))];this.rows=[];}async load(){try{this.rows=await Promise.all(this.loaders.map(fn=>fn()));}catch{this.rows=[];}this.requestUpdate();}render(){return html\`<p>\${this.rows.join(', ')||'Sin datos'}</p>\`;}}customElements.define('city-board',CityBoard);`,
  ...
  lesson({
    ...
    example: `import{LitElement,html}from'lit';class UniversalGreeting extends LitElement{static properties={name:{type:String}};constructor(){super();this.name='Visitante';}render(){return html\`<p>Hola, \${this.name}</p>\`; }connectedCallback(){super.connectedCallback();if(typeof window!=='undefined')this.clientReady=true;}}customElements.define('universal-greeting',UniversalGreeting);`,
    starter: `import{LitElement,html}from'lit';class SsrProductCard extends LitElement{static properties={name:{type:String},price:{type:Number},saved:{state:true}};constructor(){super();this.name='';this.price=0;this.saved=false;}
 connectedCallback(){super.connectedCallback();/* trabajo exclusivo del navegador con guard explícito */}
 render(){/* contenido semántico completo desde propiedades y botón que hidrata la interacción */}}
customElements.define('ssr-product-card',SsrProductCard);`,
    ...
    debug: { title: 'El componente lee window durante la definición de la clase', expected: 'viewport-card difiere la lectura y protege el entorno.', observed: 'Importar el módulo en servidor lanza ReferenceError.',
      starter: `import{LitElement,html}from'lit';const initialWidth=window.innerWidth;class ViewportCard extends LitElement{constructor(){super();this.width=initialWidth;}render(){return html\`<p>Ancho: \${this.width}</p>\`;}}customElements.define('viewport-card',ViewportCard);`,
  ...
  lesson({
    ...
    example: `import{LitElement,html}from'lit';class InventoryApp extends LitElement{static properties={items:{state:true}};constructor(){super();this.items=[];}add(name){const clean=name.trim();if(!clean)return false;this.items=[...this.items,{id:crypto.randomUUID(),name:clean,status:'active'}];return true;}render(){return html\`<button @click=\${()=>this.add('Cable')}>Añadir</button><p role="status">\${this.items.length} elementos</p>\`;}}customElements.define('inventory-app',InventoryApp);`,
    starter: `import{LitElement,html}from'lit';class TicketService{constructor(storage){this.storage=storage;}async load(){/* normaliza y devuelve tickets */}async save(tickets){/* persiste una copia */}}
class SupportCenter extends LitElement{static properties={tickets:{state:true},filter:{state:true},status:{state:true}};constructor(){super();this.tickets=[];this.filter='all';this.status='idle';this.service=new TicketService({load:async()=>[],save:async()=>{}});}
 async connectedCallback(){super.connectedCallback();/* carga con estados y recuperación */}
 async createTicket(title,priority){/* valida, actualiza inmutable y persiste */}
 async closeTicket(id){/* reemplaza ticket, persiste y conserva identidad */}
 get visibleTickets(){/* filtro derivado */}
 render(){/* formulario, filtros, estado remoto, resumen y lista accesible */}}
customElements.define('support-center',SupportCenter);`,
    ...
    tests: [browserTest('lit40-create', 'Crea y persiste un ticket válido', `async ({document,customElements})=>{await customElements.whenDefined('support-center');const el=document.querySelector('support-center');await el.createTicket('Acceso bloqueado',3);await el.updateComplete;return el.tickets.length===1&&el.tickets[0].priority===3&&el.shadowRoot.textContent.includes('Acceso bloqueado');}`), browserTest('lit40-close', 'Cerrar reemplaza el ticket correcto', `async ({document})=>{const el=document.querySelector('support-center');const id=el.tickets[0].id;const before=el.tickets[0];await el.closeTicket(id);return before!==el.tickets[0]&&el.tickets[0].status==='closed';}`), browserTest('lit40-invalid', 'Rechaza títulos vacíos y prioridades inválidas', `async ({document})=>{const el=document.querySelector('support-center');const count=el.tickets.length;await el.createTicket(' ',8);return el.tickets.length===count;}`), sourceTest('lit40-ui', 'Expone filtros y estados accesibles', String.raw`role=['"]status['"][\s\S]*(?:select|radio|button)`)],
    ...
    debug: { title: 'Guardar ocurre antes de actualizar y persiste la lista vieja', expected: 'project-board calcula next, asigna y guarda la misma versión.', observed: 'save recibe this.cards antes del cambio.',
      starter: `import{LitElement,html}from'lit';class ProjectBoard extends LitElement{constructor(){super();this.cards=[];this.storage={save:async()=>{}};}async add(title){await this.storage.save(this.cards);this.cards=[...this.cards,{id:crypto.randomUUID(),title}];}render(){return html\`<p>\${this.cards.length}</p>\`;}}customElements.define('project-board',ProjectBoard);`,
