...
export const COMPONENT_SPECS_21_TO_26 = [
  lesson({
    ...
    example: `import{LitElement,html}from'lit';class SearchForm extends LitElement{render(){return html\`<form @submit=\${this._submit}><input name="query"><button>Buscar</button></form>\`;}_submit(event){event.preventDefault();const query=new FormData(event.currentTarget).get('query');this.dispatchEvent(new CustomEvent('search',{detail:{query},bubbles:true,composed:true}));}}
customElements.define('search-form',SearchForm);`,
    starter: `import{LitElement,html}from'lit';class ProfileEditor extends LitElement{
  render(){return html\`<form><label>Nombre <input name="name"></label><button>Guardar</button><p></p></form>\`;}
  _submit(event){/* evita navegación, valida nombre y emite profile-save */}
}
customElements.define('profile-editor',ProfileEditor);`,
    ...
    debug: { title: 'El manejador se ejecuta al renderizar', expected: 'invite-form llama _submit solo al enviar.', observed: 'Usa @submit=${this._submit()}.',
      starter: `import{LitElement,html}from'lit';class InviteForm extends LitElement{render(){return html\`<form @submit=\${this._submit()}><input name="email"><button>Invitar</button></form>\`;}_submit(event){event.preventDefault();this.setAttribute('sent','');}}
customElements.define('invite-form',InviteForm);`,
  ...
  lesson({
    ...
    example: `import{LitElement,html}from'lit';class OnlineStatus extends LitElement{static properties={online:{state:true}};constructor(){super();this.online=navigator.onLine;this._sync=()=>this.online=navigator.onLine;}connectedCallback(){super.connectedCallback();window.addEventListener('online',this._sync);window.addEventListener('offline',this._sync);}disconnectedCallback(){window.removeEventListener('online',this._sync);window.removeEventListener('offline',this._sync);super.disconnectedCallback();}render(){return html\`<p>\${this.online?'En línea':'Sin conexión'}</p>\`;}}
customElements.define('online-status',OnlineStatus);`,
    starter: `import{LitElement,html}from'lit';class ConnectionMonitor extends LitElement{
  static properties={ticks:{state:true}};constructor(){super();this.ticks=0;}
  connectedCallback(){/* deja que Lit se conecte y comienza un intervalo */}
  disconnectedCallback(){/* limpia el intervalo y deja que Lit se desconecte */}
  render(){return html\`<p>Monitor: \${this.ticks}</p>\`;}
}
customElements.define('connection-monitor',ConnectionMonitor);`,
    ...
    debug: { title: 'El componente se conecta pero nunca renderiza', expected: 'broken-clock muestra su template.', observed: 'connectedCallback reemplaza el de Lit sin llamar super.',
      starter: `import{LitElement,html}from'lit';class BrokenClock extends LitElement{connectedCallback(){this.started=true;}render(){return html\`<p>Reloj iniciado</p>\`;}}customElements.define('broken-clock',BrokenClock);`,
  ...
  lesson({
    ...
    example: `import{LitElement,html}from'lit';class PriceSummary extends LitElement{static properties={items:{type:Array},total:{state:true}};constructor(){super();this.items=[];this.total=0;}willUpdate(changed){if(changed.has('items'))this.total=this.items.reduce((s,i)=>s+i.price,0);}render(){return html\`<p>Total: \${this.total}</p>\`;}}
customElements.define('price-summary',PriceSummary);`,
    starter: `import{LitElement,html}from'lit';class FilterSummary extends LitElement{
  ...
  willUpdate(changed){/* recalcula visible solo si cambia items o query */}
  render(){return html\`<p>Coincidencias: \${this.visible.length}</p>\`;}
}
customElements.define('filter-summary',FilterSummary);`,
    ...
    debug: { title: 'updated crea un bucle de actualizaciones', expected: 'tax-total calcula total sin programarse infinitamente.', observed: 'updated incrementa la misma propiedad en cada ciclo.',
      starter: `import{LitElement,html}from'lit';class TaxTotal extends LitElement{static properties={subtotal:{type:Number},total:{state:true}};constructor(){super();this.subtotal=100;this.total=0;}updated(){this.total=this.total+this.subtotal*0.1;}render(){return html\`<p>\${this.total}</p>\`;}}customElements.define('tax-total',TaxTotal);`,
  ...
  lesson({
    ...
    example: `import{LitElement,html}from'lit';class AutoFocusField extends LitElement{render(){return html\`<input aria-label="Buscar">\`;}firstUpdated(){this.renderRoot.querySelector('input').focus();}}
customElements.define('auto-focus-field',AutoFocusField);`,
    starter: `import{LitElement,html}from'lit';class FocusSearch extends LitElement{
  static properties={results:{state:true}};constructor(){super();this.results=[];}
  ...
  firstUpdated(){/* enfoca el input */}
  async search(){this.results=['Uno','Dos'];/* espera updateComplete y marca status como anunciado */}
}
customElements.define('focus-search',FocusSearch);`,
    ...
    debug: { title: 'El input es null en constructor', expected: 'search-box enfoca tras render.', observed: 'Consulta renderRoot antes de que render produzca el input.',
      starter: `import{LitElement,html}from'lit';class SearchBox extends LitElement{constructor(){super();this.renderRoot.querySelector('input').focus();}render(){return html\`<input aria-label="Consulta">\`;}}customElements.define('search-box',SearchBox);`,
  ...
  lesson({
    ...
    example: `import{LitElement,html,css}from'lit';class StatusCard extends LitElement{static styles=css\`:host{display:block;border-left:4px solid var(--status-color,#38bdf8);padding:16px}h2{margin:0}\`;render(){return html\`<h2>Servicio</h2><p>Operativo</p>\`;}}
customElements.define('status-card',StatusCard);`,
    starter: `import{LitElement,html,css}from'lit';class ThemeCard extends LitElement{
  static styles=css\`/* estiliza :host y usa --card-accent con fallback */\`;
  render(){return html\`<article><h2>Panel personal</h2><p>Tema configurable</p></article>\`;}
    ...
    debug: { title: 'El tema no cruza el shadow root', expected: 'brand-chip usa --brand-color.', observed: 'Intenta seleccionar .brand-chip desde la página.',
      starter: `import{LitElement,html,css}from'lit';class BrandChip extends LitElement{static styles=css\`span{color:blue}\`;render(){return html\`<span>Marca</span>\`;}}customElements.define('brand-chip',BrandChip);`,
  ...
  lesson({
    ...
    example: `import{LitElement,html,css}from'lit';class InfoPanel extends LitElement{static styles=css\`:host{display:block}::slotted([slot="title"]){font-weight:800}\`;render(){return html\`<section part="surface"><header><slot name="title"></slot></header><slot></slot></section>\`;}}
customElements.define('info-panel',InfoPanel);`,
    starter: `import{LitElement,html,css}from'lit';class AppPanel extends LitElement{
  static styles=css\`:host{display:block}/* estiliza slotted header sin depender de h2 */\`;
  render(){return html\`<section part="surface"><header><slot name="header"></slot></header><main><slot></slot></main><footer><slot name="actions"></slot></footer></section>\`;}
    ...
    debug: { title: 'El consumidor no puede tematizar la superficie', expected: 'report-card expone part="surface".', observed: 'La sección interna no tiene una puerta pública.',
      starter: `import{LitElement,html}from'lit';class ReportCard extends LitElement{render(){return html\`<section><slot></slot></section>\`;}}customElements.define('report-card',ReportCard);`,
