...
class OrderTray extends LitElement {
  ...
  render() {
    /* usa when para vacío y repeat con id para filas que puedan quitarse */
  }
}
...
customElements.define('issue-app', IssueApp);`,
    ...
    tests: [browserTest('lit32-create', 'Crear una incidencia produce una fila', `async ({document,customElements})=>{await customElements.whenDefined('issue-app');const el=document.querySelector('issue-app');el.createIssue('Error de acceso',2);await el.updateComplete;return el.issues.length===1&&el.shadowRoot?.textContent.includes('Error de acceso');}`), browserTest('lit32-close', 'Cerrar conserva identidad y actualiza estado', `async ({document})=>{const el=document.querySelector('issue-app');const issue=el.issues[0];if(!issue)return false;const before=issue;el.closeIssue(issue.id);await el.updateComplete;return before!==el.issues[0]&&el.issues[0].status==='closed';}`), browserTest('lit32-invalid', 'Rechaza una entrada inválida', `async ({document})=>{const el=document.querySelector('issue-app');const count=el.issues.length;el.createIssue('  ',9);await el.updateComplete;return el.issues.length===count;}`)],
