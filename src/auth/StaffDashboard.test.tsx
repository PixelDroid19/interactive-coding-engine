...
beforeEach(() => {
  vi.clearAllMocks();
  api.overview.mockResolvedValue({ learners: 1, anonymousLearners: 2, active30d: 3, completedItems: 4, anonymousCompletedItems: 5, needsSupport: 1, openThreads: 0 });
  ...
  api.learner.mockResolvedValue({
    user: learner, progress: [{ itemKey: 'fundamentos-01' }], attempts: [{ result: 'failure' }],
    skills: [{ skillKey: 'funciones', capability: 'aplicar', attempts: 3, score: 0.4 }], feedback: [],
  });
...
describe('panel de seguimiento', () => {
  ...
  it('permite revisar el recorrido y dejar feedback concreto', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('Curso (opcional, ej. fundamentos)'), { target: { value: 'fundamentos' } });
    ...
    await waitFor(() => expect(api.leaveFeedback).toHaveBeenCalledWith({
      learnerUserId: 'learner-1', courseSlug: 'fundamentos',
