...
export function StaffDashboard({ canAdmin, staffIdentity, onClose }: { canAdmin: boolean; staffIdentity: StaffDashboardIdentity | null; onClose(): void }) {
  ...
  return (
    ...
    >
      <UiSurface as="aside" tone="soft" className="staff-dashboard__rail">
      ...
      <main className="staff-dashboard__main">
        <UiSurface as="header" className="staff-dashboard__header">
...
function CourseContentEditor({ course, disabled, reload, setError }: { course: AdminCourse; disabled: boolean; reload(): Promise<void>; setError(value: string | null): void }) {
  ...
  return <div className="staff-course-content"><span>{course.slug}</span><label>Título de {course.slug}<input value={title} maxLength={160} onChange={(event) => setTitle(event.target.value)} /></label><label>Descripción de {course.slug}<textarea value={description} maxLength={1000} onChange={(event) => setDescription(event.target.value)} /></label><label>Frase corta de {course.slug}<input value={tagline} maxLength={180} onChange={(event) => setTagline(event.target.value)} /></label><UiButton variant="secondary" aria-label={`Guardar contenido de ${course.slug}`} disabled={disabled || saving || !title.trim() || !description.trim()} onClick={() => void save()}>{saving ? 'Guardando versión…' : 'Guardar contenido'}</UiButton></div>;
}
