...
const PROPOSAL_STATUS_LABEL: Record<ImprovementProposal['status'], string> = {
  open: 'Abierta', queued: 'En cola', building: 'Construyendo', preview: 'Desplegado automáticamente',
...
const RUN_STATUS_LABEL: Record<AdminImprovementProposal['runs'][number]['status'], string> = {
  queued: 'en cola', running: 'construyendo', succeeded: 'validado y desplegado', rejected: 'rechazado',
...
export function ImprovementCenter({ canAdmin, onClose }: { canAdmin: boolean; onClose(): void }) {
  ...
  async function syncReview(item: ImprovementProposal) {
    ...
    catch (cause) { setError(cause instanceof Error ? cause.message : 'No pudimos sincronizar el estado del despliegue.'); }
    finally { setBusy(null); }
  }
          ...
          {items.map((item) => {
            ...
            return <UiSurface as="article" className="improvement-card" key={item.id}>
              ...
              <footer>
                ...
                {canAdmin && item.status === 'open' && <UiButton variant="primary" disabled={Boolean(busy)} onClick={() => void queue(item)}><Bot size={17} /> Implementar automáticamente con Muse</UiButton>}
              </footer>
              {latestRun && <div className="improvement-run">
                <strong>{latestRun.status === 'succeeded' ? 'Validado y desplegado automáticamente' : `Muse: ${RUN_STATUS_LABEL[latestRun.status]}`}</strong>
                ...
                {canAdmin && item.status === 'preview' && pullRequestUrl && <UiButton variant="secondary" disabled={Boolean(busy)} onClick={() => void syncReview(item)}><RefreshCw size={15} /> Sincronizar estado del despliegue</UiButton>}
